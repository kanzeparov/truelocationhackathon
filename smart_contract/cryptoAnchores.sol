...
//    uint time;
    ...
    function checkSignature(uint8 v, bytes32 r, bytes32 s, bytes32 signedHash, address UnitId) public view returns (address)
    {
    ...
    function UpdateState (string _status, uint256 _latitude, uint256 _longitude, uint8 v, bytes32 r, bytes32 s, bytes32 signedHash, address UnitId) public
    {
        ...
        bytes32 prefixedHash = sha3(prefix, signedHash);
        signer = ecrecover(prefixedHash, v, r, s);
        require(UnitId == signer);
